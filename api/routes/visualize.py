...
async def visualize_run(
    ...
    return _render_html(run_data)


def _render_html(run_data: dict) -> str:
    def esc(text):
        if text is None:
            return ""
    ...
    started = run_data.get('started_at', '')[:19].replace('T', ' ') if run_data.get('started_at') else 'N/A'
    completed = run_data.get('completed_at', '')[:19].replace('T', ' ') if run_data.get('completed_at') else None
